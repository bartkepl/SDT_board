...
{
    HAL_GPIO_WritePin(CU_GPIO_Port, CU_Pin, state);
}
...
{
    setCU(GPIO_PIN_SET); // COMMAND mode

    setData(cmd);

    pulseCUE(); // latch command
}

void DLR2416_Init(void)
{
    HAL_TIM_PWM_Start(&htim3, TIM_CHANNEL_2);
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, 599);

    HAL_GPIO_WritePin(WR_GPIO_Port, WR_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(CUE_GPIO_Port, CUE_Pin, GPIO_PIN_SET);

    setCU(GPIO_PIN_RESET); // start w trybie DATA
}
