...
```python
print(inst.query('DISPlay:BRIGhtness?'))   # 4
inst.write('DISPlay:BRIGhtness 50')         # 50% jasności
inst.write('DISPlay:BRIGhtness 100')        # maksymalna jasność
```

!!! info "Implementacja PWM"
    Jasność sterowana przez TIM3 CH2 (PC14, AF11). Formuła: `CCR = (1199 × percent) / 100`.  
    TIM3 ARR = 255, więc CCR > 255 → zawsze HIGH. Efektywna regulacja działa dla brightness ≤ 21%,
    powyżej wyświetlacz jest zawsze w pełni jasny.
